...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, embedding_dim=512, n_heads=8):
        ...
        self.softmax = Softmax(-1, keepdim=True) # Custom Softmax layer
    ...
    def _scaled_dot_product_attention(self, query, key, value, mask=None, scale=True):
        ...
        scores = self.softmax(dots / torch.sqrt(torch.tensor(depth, dtype=torch.long))) # perform softmax operation dot product scaled by the scaling factor
        ...
        weights = weights.permute(0, 2, 1, 3).contiguous() # Swapping the second and first fimension of the weights matrix. resulting matrix has a shape of [batch_size, v_Len, heads, head_size]
    ...
    def forward(self, query, key, value, mask=None, return_attention=False):
        ...
        weights, attention = self._scaled_dot_product_attention(query, key, value, mask=mask) # [batch_size, v_len, heads, head_size]
